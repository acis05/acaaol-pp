...
function showResultPanel(summary = {}, results = []) {
  ...
  if (failedItems.length > 0) {
    const lines = [];
    lines.push("CATATAN GAGAL IMPORT PURCHASE PAYMENT");
    lines.push("==================================");
    failedItems.forEach((x, idx) => {
      lines.push("");
      lines.push(`${idx + 1}. Payment: ${x.number || "-"} | Tanggal: ${x.transDate || "-"} | Vendor: ${x.vendorNo || "-"}`);
...
if ($("btnLoadDb")) {
  $("btnLoadDb").onclick = async () => {
    try {
      ...
      if (arr.length === 0) {
        showNotify("info", "Database tidak dinvoiceukan", renderSimpleMessage(["Tidak ada database yang muncul di akun ini."]));
      } else {
...
if ($("btnBuild")) {
  $("btnBuild").onclick = async () => {
    try {
      ...
      const res = await postForm("/api/build-purchase-payment", fd);

      builtPayload = res.payload;

      setSummary(`File siap diimport: ${res.summary.transactions} payment, ${res.summary.lines} invoice.`);
      showResultPanel({
      ...
      showNotify(
        ...
        renderSimpleMessage([
          ...
          `Jumlah invoice: ${res.summary.lines}`,
...
if ($("btnImport")) {
  $("btnImport").onclick = async () => {
    try {
      ...
      setSummary("Sedang mengimport Purchase Payment ke Accurate...");
      const res = await postJson("/api/import-purchase-payment", { payload: builtPayload }, true);
